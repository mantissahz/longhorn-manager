...
func (b *backupTargetValidator) validateDRVolume(backupTarget *longhorn.BackupTarget) error {
	...
	backupTargetURL, err := getBackupTargetURLWithoutOpts(backupTarget.Spec.BackupTargetURL)
	if err != nil {
		return err
	}
	if len(vs) != 0 {
		...
		for k, v := range vs {
			backupTargetURLfromBackupURL, err := getBackupTargetURLWithoutOpts(v.Spec.FromBackup)
			if err != nil {
				return err
			}
			fromBackupTargetName, ok := v.Labels[types.LonghornLabelBackupTarget]
			if (ok && fromBackupTargetName == backupTarget.Name) || backupTargetURLfromBackupURL == backupTargetURL {
...
func getBackupTargetURLWithoutOpts(inputURL string) (string, error) {
	parsedURL, err := url.Parse(inputURL)
	if err != nil {
		return "", errors.Wrapf(err, "failed to parse URL %v", inputURL)
	}
	parsedURL.RawQuery = ""
...
func (b *backupTargetValidator) Delete(request *admission.Request, oldObj runtime.Object) error {
	...
	if backupTarget.Annotations == nil {
		// Annotations are nil, so the key does not exist
		exists = false
	} else {
		_, exists = backupTarget.Annotations[types.GetLonghornLabelKey(types.DeleteBackupTargetFromLonghorn)]
	}
	if backupTarget.Name == types.DefaultBackupTargetName && !exists {
