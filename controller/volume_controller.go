...
func (c *VolumeController) getBackupVolumeInfo(v *longhorn.Volume) (string, string, string, error) {
	...
	backupVolume, err := c.ds.GetBackupVolumeByBackupTargetAndVolumeRO(backup.Status.BackupTarget, canonicalBVName)
	if err != nil {
		return "", "", "", errors.Wrapf(err, "failed to get backup volume %v with backup target %v for restoring volume %v", canonicalBVName, backup.Status.BackupTarget, v.Name)
	}
