...
func (bc *BackupController) reconcile(backupName string) (err error) {
	...
	if !backup.DeletionTimestamp.IsZero() {
		...
		backupVolume, err := bc.ds.GetBackupVolumeByBackupTargetAndVolume(backupTargetName, remoteBackupVolumeName)
		...
		if needsCleanupRemoteData && backupVolume != nil && backupVolume.DeletionTimestamp == nil {
			...
			backupDeleted := bc.handleBackupDeletionInBackupStore(backup, remoteBackupVolumeName, backupURL, backupTargetClient)
			if !backupDeleted {
				return nil
			}
		}
	...
	backup.Status.BackupTarget = backupTargetName
